const EMBED_REPLACEMENTS = {
  ...
  "Unit8-FigmaPP": {
    type: "embed",
    src: "https://embed.figma.com/slides/dXeqnHS4e9UQvtnL7JdCXw/FMP-Pitch?node-id=1-105&embed-host=share",
...
export function replaceEmbeds(content) {
  let out = content;
  for (const [key, val] of Object.entries(EMBED_REPLACEMENTS)) {
    if (val.type === "model") {
      out = out.replaceAll(key, modelEmbedBlock(val.src, val.linkText));
    } else if (val.type === "audio") {
      out = out.replaceAll(key, audioEmbedBlock(val.src, val.linkText));
    } else {
      out = out.replaceAll(key, embedBlock(val.src, val.linkText));
    }
  }
  return out;
}
