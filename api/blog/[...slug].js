...
const WP_API_BASE = "https://public-api.wordpress.com/wp/v2/sites/kingowenblog.wordpress.com";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const title = post?.title?.rendered ? stripHtml(post.title.rendered) : "Blog - Owen";
    const descRaw = post?.excerpt?.rendered
      ? stripHtml(post.excerpt.rendered)
      : "Read my latest blog posts and project updates.";
    const description = descRaw || "Read my latest blog posts and project updates.";
    ...
    const canonicalUrl = `${proto}://${host}/blog/${encodeURIComponent(slug)}`;
    ...
    if (!html) {
      try {
        ...
        html = "<!doctype html><title>Blog</title>";
      }
    }
